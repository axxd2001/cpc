...
#if defined(NDEBUG)
# error "Chinesepaintingcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Chinesepaintingcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Chinesepaintingcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Chinesepaintingcoin: find block wanted by given orphan block
...
// Chinesepaintingcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Chinesepaintingcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Chinesepaintingcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Chinesepaintingcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Chinesepaintingcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Chinesepaintingcoin: track money supply and mint amount info
...
// Chinesepaintingcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Chinesepaintingcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Chinesepaintingcoin: compute stake entropy bit for stake modifier
    ...
    // Chinesepaintingcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Chinesepaintingcoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Chinesepaintingcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Chinesepaintingcoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Chinesepaintingcoin-loadblk");
